...
if __name__=="__main__":
    ...
    model.to(device)
    model.eval()
    x = torch.ones(1, 1, 512, 300).to(device)
    modelName = "VGGVox_full_pretrained" + ".onnx"
    out = model.forward(x)
    print(out.shape)
    torch.onnx.export(model,x,modelName,opset_version=9,input_names = ['input'], output_names = ['output'])
